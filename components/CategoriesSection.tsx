...
export function CategoriesSection() {
  ...
  const nextSlide = () => {
    setCurrentSlide(
      (prev) => (prev + 1) % Math.ceil(categories.length / 2),
    );
  };
  ...
  const prevSlide = () => {
    setCurrentSlide(
      (prev) =>
        (prev - 1 + Math.ceil(categories.length / 2)) %
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-5xl font-light text-black mb-4">
            <span className="font-bold">Variedade</span> que
            você precisa
          </h2>
          <p className="text-lg text-black/70 max-w-2xl mx-auto">
            Descubra a amplitude de categorias e serviços
            disponíveis no app
          </p>
        </div>
        ...
        {isMobile ? (
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {categories
                          ...
                          .map((category, index) => (
                            <div
                              key={index}
                              className={`${category.color} rounded-xl overflow-hidden border group`}
                            >
                                  ...
                                  {category.services
                                    ...
                                    .map(
                                      (
                                        ...
                                        <span
                                          key={serviceIndex}
                                          className="bg-white/70 text-black/70 text-xs px-2 py-1 rounded-full text-center"
                                        >
                                          {service}
                                        </span>
                                      ),
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((category, index) => (
              <div
                key={index}
                className={`${category.color} rounded-2xl overflow-hidden border group hover:shadow-lg transition-all duration-300`}
              >
                {/* Header with image and icon */}
        ...
        <div className="mt-12 text-center">
          <p className="text-lg text-black/70 max-w-2xl mx-auto">
            ...
            Cada categoria tem dezenas de subcategorias e
            centenas de profissionais verificados pela
            comunidade.
          </p>
        </div>
      </div>
